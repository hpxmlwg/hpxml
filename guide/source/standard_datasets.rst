...
The HEScore API has a built-in translator, developed by the National Laboratory of the Rockies (NLR), that can generate HEScore inputs from HPXML files. As such, software developers can leverage their investment in HPXML to provide HEScore functionality with minimum development cost.
